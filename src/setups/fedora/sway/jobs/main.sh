...
run_jobs() {
  ...
  local SHARED_FEDORA_JOBS_DIR="$ROOT_DIR_LOCAL/src/setups/fedora/shared/jobs"
  ...
  for job in "$SHARED_FEDORA_JOBS_DIR/common"/*.sh; do
    [ -e "$job" ] || continue
    echo "=> Running $job"
    bash "$job"
  done

  for job in "$SHARED_FEDORA_JOBS_DIR/common/for-clean-systems"/*.sh; do
    [ -e "$job" ] || continue
    echo "=> Running $job"
    bash "$job"
  done
